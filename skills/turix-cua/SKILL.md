---
name: turix-cua
description: Computer Use Agent (CUA) for cross-platform desktop automation using TuriX. Supports macOS, Windows, and Linux. Use for visual desktop tasks like opening apps, clicking buttons, or navigating UIs without CLI/API.
---

# TuriX-CUA Skill (Cross-Platform)

This skill allows Clawdbot to control the desktop visually using the TuriX Computer Use Agent.

## 🐧 Linux Support (NEW!)

**Status:** ✅ Beta - Community Adaptation

Linux version uses `pyautogui` + `pynput` + `xdotool` for cross-platform compatibility.

**Setup:**
```bash
# Install dependencies
pip install pyautogui pynput
sudo apt install xdotool scrot  # Optional but recommended

# Run on Linux
skills/local/turix-cua/scripts/run_turix_linux.sh "Your task"
```

**Notes:**
- First run may take 2-5 minutes to load AI models
- Requires X11 display (`export DISPLAY=:0`)
- Screenshot permissions should work by default on X11
